...
export class VonageAdapter extends BaseVoipAdapter {
  ...
  generateCallResponse(response: CallControlResponse): string {
    ...
    switch (response.action) {
      ...
      case 'stream':
        // If we have initialAudio (greeting), we could use 'stream' action to play it
        // However, Vonage requires audio URL, not inline Base64
        // For now, greeting will be sent via WebSocket on stream connect
        if (response.initialAudio) {
          // TODO: In production, upload Base64 audio to S3/CDN and get URL
          // Then use: { action: 'stream', streamUrl: audioUrl } before connect
