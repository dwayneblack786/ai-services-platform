...
<Response>
  <Transfer>
    <PhoneNumber>${response.forwardTo}</PhoneNumber>
      ...
      case 'stream':
        // If we have initialAudio (greeting), we could use PlayAudio to play it
        // However, Bandwidth requires audio URL, not inline Base64
        // For now, greeting will be sent via stream on connect
        if (response.initialAudio) {
          // TODO: In production, upload Base64 audio to S3/CDN and get URL
          // Then use: <PlayAudio>${audioUrl}</PlayAudio> before StartStream
