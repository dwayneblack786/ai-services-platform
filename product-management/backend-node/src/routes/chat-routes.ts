...
router.post('/session', authenticateSession, async (req: Request, res: Response) => {
  try {
    ...
    if (!forceNew) {
      try {
        const activeSessionCheck = await javaVAClient.get<{ hasActiveSession: string; sessionId: string }>(
...
router.get('/message/stream', authenticateSession, streamRateLimiter, async (req: Request, res: Response) => {
  try {
    ...
    // Convert Web Streams API ReadableStream to async iteration
    if (javaResponse.body) {
      const reader = javaResponse.body.getReader();
...
// POST /chat/end
// End a chat session
router.post('/end', authenticateSession, async (req: Request, res: Response) => {
  try {
...
router.get('/history/:sessionId', authenticateSession, async (req: Request, res: Response) => {
  try {
...
router.get('/rate-limit/stats', authenticateSession, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id || (req as any).user?.email || 'anonymous';
