  ...
  try {
    ...
    try {
      ...
      if (sessionInitResponse.ok) {
        const sessionData = await sessionInitResponse.json() as { sessionId: string; greetingText: string | null; greetingAudio: string | null };
    ...
    console.log('[Voice Incoming] Streaming to:', streamUrl, '| Greeting stored in DB:', !!greetingAudio);
