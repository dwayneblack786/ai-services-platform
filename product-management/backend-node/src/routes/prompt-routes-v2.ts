...
router.get('/industry/:industry', authenticateSession, async (req: Request, res: Response) => {
  try {
...
router.get('/customer/:id', authenticateSession, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
