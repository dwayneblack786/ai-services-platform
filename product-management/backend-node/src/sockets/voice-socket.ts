...
export function setupVoiceHandlers(socket: AuthenticatedSocket): void {
  ...
  socket.on('voice:session:init', async (data: VoiceSessionInitEventData) => {
    ...
    console.log(`[Voice] 🎬 Initializing voice session:`, {
    ...
    try {
      ...
      if (response.greetingText && response.greetingAudio) {
        ...
        status = 'ready';
        console.log(`[Voice] 🎙️ Greeting generated:`, {
          textPreview: response.greetingText.substring(0, 60) + '...',
          audioSize: response.greetingAudio.length,
          estimatedKB: Math.round(response.greetingAudio.length * 3 / 4 / 1024)
        });
        ...
        console.log(`[Voice] ⚠️ Text-only greeting (TTS unavailable):`, response.greetingText.substring(0, 60));
      } else {
        console.log(`[Voice] ⚠️ No greeting generated (LLM unavailable)`);
      }
