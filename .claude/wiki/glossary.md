...
- The workspace root (`ai-services-platform/`) is a workspace-scoped development repository. It owns `.claude/`, `docs/`, `plans/`, `podman-compose.yml`, root `CLAUDE.md`, and shared scripts. It is not a product repository.
- `services-java`, `product-management`, `ai-listing-agent`, and `shared` are independent product repositories.
