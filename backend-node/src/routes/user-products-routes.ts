...
router.get('/', async (req, res) => {
  try {
    ...
    const db = getDB();
    const userProducts = await db.collection<UserProduct>('user_products')
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingSubscription = await db.collection<UserProduct>('user_products')
      .findOne({ userId, productId, status: 'active' });
    ...
    const newSubscription: Omit<UserProduct, '_id'> = {
      userId,
      productId,
    ...
    const result = await db.collection<UserProduct>('user_products')
    ...
    res.json({ 
      success: true, 
      userProduct: { ...newSubscription, _id: result.insertedId }
    });
  } catch (error: any) {
